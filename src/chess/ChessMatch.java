...
public class ChessMatch {
	...
	private void validateSourcePosition(Position position) {
		...
		if (currentPlayer != ((ChessPiece) board.piece(position)).getColor()) {
			throw new ChessException(String.format("%s player can't move %s pieces.", currentPlayer,
					((ChessPiece) board.piece(position)).getColor()));
		}
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> piecesList = piecesOnTheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
				.collect(Collectors.toList());
